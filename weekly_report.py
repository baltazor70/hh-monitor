...
rows = conn.execute("""
    ...
    WHERE published_date BETWEEN ? AND ? AND lower(name) NOT LIKE '%руководитель ит отдела%' AND lower(name) NOT LIKE '%руководитель it отдела%' AND lower(name) NOT LIKE '%начальник ит отдела%' AND lower(name) NOT LIKE '%руководитель отдела информационных технологий%' AND salary_currency IN ('RUB','RUR')
""", (ws_iso, we_iso)).fetchall()

total = conn.execute("SELECT COUNT(*) as c FROM vacancies WHERE published_date BETWEEN ? AND ? AND lower(name) NOT LIKE '%руководитель ит отдела%' AND lower(name) NOT LIKE '%руководитель it отдела%' AND lower(name) NOT LIKE '%начальник ит отдела%' AND lower(name) NOT LIKE '%руководитель отдела информационных технологий%'", (ws_iso, we_iso)).fetchone()['c']
