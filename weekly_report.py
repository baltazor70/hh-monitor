...
rows = conn.execute("""
    ...
    WHERE published_date BETWEEN ? AND ? AND (lower(name) LIKE '%поддержк%' OR lower(name) LIKE '%support%' OR lower(name) LIKE '%service desk%' OR lower(name) LIKE '%helpdesk%' OR lower(name) LIKE '%customer success%' OR lower(name) LIKE '%customer service%' OR lower(name) LIKE '%клиентского сервиса%' OR lower(name) LIKE '%itsm%' OR lower(name) LIKE '%сервис%' OR lower(name) LIKE '%service%' OR lower(name) LIKE '%care%') AND lower(name) NOT LIKE '%руководитель ит отдела%' AND lower(name) NOT LIKE '%руководитель it отдела%' AND lower(name) NOT LIKE '%начальник ит отдела%' AND lower(name) NOT LIKE '%руководитель отдела информационных технологий%' AND salary_currency IN ('RUB','RUR')
