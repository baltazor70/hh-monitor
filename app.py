...
def stats():
    ...
    counts_rows = conn.execute("""
        ...
        FROM vacancies WHERE published_date >= ? AND (lower(name) LIKE '%поддержк%' OR lower(name) LIKE '%support%' OR lower(name) LIKE '%service desk%' OR lower(name) LIKE '%helpdesk%' OR lower(name) LIKE '%customer success%' OR lower(name) LIKE '%customer service%' OR lower(name) LIKE '%клиентского сервиса%' OR lower(name) LIKE '%itsm%' OR lower(name) LIKE '%сервис%' OR lower(name) LIKE '%service%' OR lower(name) LIKE '%care%' OR lower(name) LIKE '%мониторинг%' OR lower(name) LIKE '%1 линии%' OR lower(name) LIKE '%2 линии%' OR lower(name) LIKE '%1-й линии%' OR lower(name) LIKE '%2-й линии%' OR lower(name) LIKE '%первой линии%' OR lower(name) LIKE '%второй линии%' OR lower(name) LIKE '%1 line%' OR lower(name) LIKE '%2 line%') AND lower(name) NOT LIKE '%руководитель ит отдела%' AND lower(name) NOT LIKE '%руководитель it отдела%' AND lower(name) NOT LIKE '%начальник ит отдела%' AND lower(name) NOT LIKE '%руководитель отдела информационных технологий%' AND archived=0
